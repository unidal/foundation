...
public interface PlexusContainer {
	 ...
	 *
	 * Component instance is not associated with any class realm and will be ignored
	 * during lookup is lookup realm is provided using thread context classloader.
	 * 
	 * @param component
	 ...
	 * This container should also remove itself from the container hierarchy.
	 */
	void dispose();

	/**
	 * Returns this container's context. A Context is a simple data store used to
	 * hold values which may alter the execution of the Container.
	 * 
	 ...
	 * @throws ComponentLookupException when error
	 */
	<T> T lookup(Class<T> type) throws ComponentLookupException;

	/**
	 * Looks up and returns a component object with the given unique role/role-hint
	 * combination.
	 * 
	 * @param type
	 *            the non-unique type of the component
	...
	/**
	 * Looks up and returns a Map of component objects with the given role, keyed by
	 * all available role-hints.
	 * 
	 * @param type
	 *            the non-unique type of the components
	...
	/**
	 * Releases the component from the container. This is dependent upon how the
	 * implementation manages the component, but usually enacts some standard
	 * lifecycle shutdown procedure on the component. In every case, the component
	 * is no longer accessible from the container (unless another is created).
	 * 
	 * @param component
